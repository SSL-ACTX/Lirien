...
#[test]
fn test_borrow_checker_branches() {
    let source = "
...
#[test]
fn test_borrow_checker_use_after_branch_move() {
    let source = "
...
#[test]
fn test_borrow_checker_loop_move() {
    let source = "
...
#[test]
fn test_borrow_checker_aliasing_multiple_refs_ok() {
    let source = "
...
#[test]
fn test_borrow_checker_aliasing_mut_and_ref_err() {
    let source = "
def mut_and_ref(x: i64) -> i64:
    m = Mut(x)
    r = Ref(x) # Error: x is already mutably borrowed
    consume_owned(m)
    return 1
"
    .to_string();
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("already borrowed mutably"));
}

#[test]
fn test_borrow_checker_aliasing_mut_and_mut_err() {
    let source = "
def mut_and_mut(x: i64) -> i64:
    m1 = Mut(x)
    m2 = Mut(x) # Error: x is already mutably borrowed
    consume_owned(m1)
    return 1
"
    .to_string();
