# Project Lirien: Development Rules & Architecture
...
*   **Examples:**
    *   ```
        ...
        - Updated type parsing in metadata.rs to yield Type::Optional for non-pointer-like types
        - Implemented type propagation for Type::Optional under StructLoad and StructSet
        - Updated visit_compare in binary.rs to load the 1-byte boolean has_value tag
        - Added unit tests in test_null_safety.py to verify compiler, verifier, and FFI
        ```
    *   ```
        feat(compiler): implement JIT kernel fusion for element-wise tensor operations
        
        - Defined FusedExpr and InstructionKind::TensorFused in lirien-ir instruction macros
        - Implemented tensor kernel fusion pass in lirien-ir
