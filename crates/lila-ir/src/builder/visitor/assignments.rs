                ...
                match arr_ty {
                    ...
                    Type::TypedDict(ref dict_name) => {
                        ...
                        if let Some(key) = key_val {
                            let dict_name_cloned = dict_name.clone();
                            let key_cloned = key.clone();
                            let field_offset = self.get_field_offset(dict_name, &key).ok_or_else(|| {
                                builder_error!(AttributeNotFound, dict_name_cloned, key_cloned)
                            })?;
            ...
            ast::Expr::Attribute(attr) => {
                ...
                if is_subscript_base {
                    ...
                    let (offset, leaf_ty) = if let Type::Struct(struct_name) = &base_ty {
                        let struct_name_cloned = struct_name.clone();
                        let attr_name = attr.attr.to_string();
                        let field_offset = self
                            .get_field_offset(struct_name, attr.attr.as_str())
                            .ok_or_else(|| builder_error!(AttributeNotFound, struct_name_cloned, attr_name))?;
            ...
            ast::Expr::Attribute(attr) => {
                ...
                if let Type::Struct(struct_name) = curr_ty {
                    ...
                    let field_offset = self
                        .get_field_offset(struct_name, attr.attr.as_str())
                        .ok_or_else(|| {
                            builder_error!(AttributeNotFound, struct_name_cloned, attr_name)
                        })?;
